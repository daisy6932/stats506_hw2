...
setGeneric("getMean", function(x) standardGeneric("getMean"))
setMethod("getMean", "waldCI", function(x) x@mean)
...
# 1) Getter: mean(obj)
# define generic if not already defined (avoid removing base mean)
if (!isGeneric("mean")) {
  setGeneric("mean", function(x, ...) standardGeneric("mean"))
}
setMethod("mean", "waldCI", function(x, ...) x@mean)

# 2) Replacement generic: mean<- (must be defined before setReplaceMethod)
setGeneric("mean<-", function(x, value) standardGeneric("mean<-"))

# 3) Replacement method: mean(obj) <- value
setReplaceMethod("mean", "waldCI", function(x, value) {
  if (!is.numeric(value) || any(!is.finite(value))) stop("mean must be finite numeric")
  new("waldCI", mean = as.numeric(value), sterr = x@sterr)
})
...
setReplaceMethod("lb", "waldCI", function(x, ..., value) {
  dots <- list(...)
  # 如果用户没传 level，就默认 0.95
  level <- if (length(dots) >= 1 && is.numeric(dots[[1]])) dots[[1]] else 0.95
  
  z <- qnorm(1 - (1 - level) / 2)
  ubv <- ub(x, level)
  new_mean <- (value + ubv) / 2
  new_sterr <- (ubv - value) / (2 * z)
...
# Load NYTimes data (daily rolling averages)
...
covid <- covid %>%
  mutate(date = as.Date(date),
         new_cases_per_100k = cases_avg_per_100k)
```
...
national <- covid %>%
  group_by(date) %>%
  summarize(avg_cases_per_100k = mean(new_cases_per_100k, na.rm = TRUE))

# Plotly line with spike annotations
fig1 <- plot_ly(national, x = ~date, y = ~avg_cases_per_100k, type = 'scatter', mode = 'lines',
                line = list(color = 'firebrick', width = 2)) %>%
  layout(
    title = "US COVID-19 New Case Rate Over Time (7-day avg per 100k)",
    xaxis = list(title = "Date"),
    yaxis = list(title = "Average New Cases per 100k"),
    hovermode = "x unified",
    annotations = list(
      list(x = as.Date("2020-04-01"), y = 10, text = "First Wave", showarrow = TRUE, font = list(size = 12)),
      list(x = as.Date("2021-01-10"), y = 80, text = "Winter Surge", showarrow = TRUE, font = list(size = 12)),
      list(x = as.Date("2022-01-15"), y = 130, text = "Omicron Peak", showarrow = TRUE, font = list(size = 12))
    )
  )
...
```
5 noticeable waves: First Wave (Spring 2020), Winter Surge (2020-21), Delta Wave (Mid-2021), Omicron Peak (Early 2022), and Late 2022 decline.

Your annotated plot clearly highlights these events with arrows and clean labeling — excellent clarity and interactivity.
# (b)
```{r}
# Compute cumulative mean rates
state_rates <- covid %>%
  group_by(state) %>%
  summarize(mean_rate = mean(new_cases_per_100k, na.rm = TRUE)) %>%
  arrange(desc(mean_rate))

top3 <- head(state_rates$state, 3)
bottom3 <- tail(state_rates$state, 3)

compare_states <- covid %>%
  filter(state %in% c(top3, bottom3))

# Plotly grouped line chart
fig2 <- plot_ly(compare_states,
                x = ~date,
                ...
                mode = 'lines') %>%
  layout(
    title = "COVID Trajectories: States with Highest and Lowest Mean Case Rates",
    xaxis = list(title = "Date"),
    yaxis = list(title = "New Cases per 100k (7-day avg)"),
    legend = list(title = list(text = "State"))
  )

fig2 <- fig2 %>% layout(colorway = RColorBrewer::brewer.pal(6, "Dark2"))

fig2
```
Alaska, Rhode Island, American Samoa show extreme spikes (likely due to smaller populations and delayed outbreaks).

Maryland, Maine, Oregon remain smoother and less extreme.

The interactivity (hover to view exact values and zoom by time window) makes it easy to visually compare their trajectories.
# (c)
```{r}
first_wave <- covid %>%
  filter(new_cases_per_100k > 5) %>%
  group_by(state) %>%
  summarize(first_substantial = min(date, na.rm = TRUE)) %>%
  arrange(first_substantial)

first5 <- head(first_wave, 5)$state

# Plot these 5 states
early_states <- covid %>% filter(state %in% first5)

fig3 <- plot_ly(early_states,
                x = ~date,
                y = ~new_cases_per_100k,
                color = ~state,
                type = 'scatter',
                mode = 'lines') %>%
  layout(
    title = "First Five States to Experience Substantial COVID Outbreaks",
    xaxis = list(title = "Date"),
    yaxis = list(title = "New Cases per 100k (7-day avg)"),
    legend = list(title = list(text = "State"))
  )
...
fig3
```
New York, New Jersey, Massachusetts, Connecticut, and Louisiana were among the earliest states to experience substantial outbreaks (consistent with early pandemic epicenters).

Plotly’s interactivity shows when each state first surged, supporting the narrative of early spread on the East Coast.
